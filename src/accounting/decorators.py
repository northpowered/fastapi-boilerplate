...
def AAA_endpoint_oauth2():
    ...
    def outer_wrapper(func):
        # Need for openapi schema, to show, that endpoint needs a permission
        func.__setattr__('rbac_enable', True)
        ...
        async def wrapper(request: Request, *args, **kwargs):
            ...
            try:
                ...
                logger.debug(
                    f'Access denied | User: {current_user.username} | Object: {func.__name__}')
                raise PermissionDeniedException(str(ex))
            except IndexError:
                raise UnauthorizedException('Cannot decode token')
            else:
                logger.debug(
                    f'Access permitted | User: {current_user.username} | Object: {func.__name__}')
